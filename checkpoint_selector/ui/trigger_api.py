...
@server.PromptServer.instance.routes.post('/craftgear/checkpoint_preview')
async def load_checkpoint_preview(request: web.Request) -> web.StreamResponse:
    try:
        data: dict[str, Any] = await request.json()
    except Exception:
        data = {}
    ckpt_name = data.get('ckpt_name') if isinstance(data, dict) else ''
    if not ckpt_name or ckpt_name == 'None':
        return web.json_response({'ok': False, 'error': 'invalid_checkpoint'}, status=400)
...
@server.PromptServer.instance.routes.post('/craftgear/open_checkpoint_folder')
async def open_checkpoint_folder(request: web.Request) -> web.Response:
    ...
    ckpt_name = data.get('ckpt_name') if isinstance(data, dict) else ''
    if not ckpt_name or ckpt_name == 'None':
        return web.json_response({'ok': False, 'error': 'invalid_checkpoint'})
    ckpt_path = folder_paths.get_full_path('checkpoints', ckpt_name)
    if not ckpt_path or not os.path.exists(ckpt_path):

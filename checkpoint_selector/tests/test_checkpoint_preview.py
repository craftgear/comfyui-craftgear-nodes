...
from checkpoint_selector.logic.checkpoint_preview import (
    DEFAULT_IMAGE_EXTENSIONS,
    select_checkpoint_preview_path,
)


class CheckpointPreviewSelectionTest(unittest.TestCase):
    ...
    def test_returns_none_when_no_images(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ckpt_path = os.path.join(temp_dir, "alpha.safetensors")
            self._touch(ckpt_path)
            result = select_checkpoint_preview_path(
    ...
    def test_returns_none_when_path_missing(self) -> None:
        self.assertIsNone(
            select_checkpoint_preview_path("", DEFAULT_IMAGE_EXTENSIONS)
        )
    ...
    def test_returns_none_when_base_name_empty(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIsNone(
                select_checkpoint_preview_path(path, DEFAULT_IMAGE_EXTENSIONS)
            )

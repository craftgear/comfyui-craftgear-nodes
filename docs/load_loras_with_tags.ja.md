...
- 最大20個のLoRAを同時に適用できます。
- ヘッダーに2つのアクションがあります:
  - **Copy**: 他ノード（LoRA Loader / Power LoRA Loader / Load LoRAs With Tags など）からスロットを読み込み、現在のスロットを置き換えます。
  - **Append**: 他ノードから読み込んだスロットを末尾に追加し、既存のスロットを上書きしません。

...
## 概要
**Load Loras With Tags / Commentable Multiline Text / Toggle Tags** などが生成するプロンプトやタグ情報を A1111 互換の `parameters` としてまとめ、PNG/WebP 出力へ保存する用途を想定しています。`OUTPUT_NODE = True` なので出力が未接続でも実行されます。
...
- `format`（`png`/`webp` トグル, 既定 `png`）: 出力形式の切り替え。実行時は不正値が来ても `png` にフォールバックします。
...
2. `overwrite` フラグに応じて保存先パスを決定します。  
   ...
   - `overwrite=True` の場合は `format` と同じ拡張子の最新ファイルのみを上書き対象にします。
3. 形式ごとにメタデータを書き込みます。  
   - `png`: `prompt` / `parameters` / `extra_pnginfo`（予約キー除外）を PNG テキストチャンクとして保存します。  
   - `webp`: A1111 互換を優先し、EXIF `UserComment` に `parameters` のみを書き込みます。

## 使い方
1. 画像の直後に配置し、必要に応じて出力を他ノードへ接続します（未接続でも実行されます）。
2. `format` の `png/webp` トグルで出力形式を選択します。
3. 既存画像を上書きしたい場合は `overwrite` をオンにし、`SaveImage` ノードと同じプレフィックスを使ってください。
4. 複製保存したい場合は `overwrite` をオフにし、`suffix` で名前を調整します。
...
- `webp` 保存時は `prompt` と `extra_pnginfo` を埋め込まず、`parameters` のみを保持します。
